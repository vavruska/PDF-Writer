 ...
 * TD move text current point16_t and set leading.
...
#else
#include <types.h>
#define ssize_t unsigned long long
...
typedef struct pdf_object {
    int16_t type;                /* See OBJ_xxxx */
    int16_t index;               /* PDF output index */
    int16_t offset;              /* Byte position within the output file */
...
#define MIN_SHIFT 10LL
#define MIN_OFFSET ((1 << MIN_SHIFT) - 1)
static long long bin_offset[] = {
...
static inline int16_t flexarray_get_bin(const struct flexarray *flex, int16_t index)
{
    (void)flex;
...
static inline int16_t flexarray_get_bin_size(const struct flexarray *flex,
                                         int16_t bin)
{
    (void)flex;
    if (bin >= (int)ARRAY_SIZE(bin_offset) - 1)
        return -1;
    int16_t next = bin_offset[bin + 1];
    return next - bin_offset[bin];
}

static inline int16_t flexarray_get_bin_offset(const struct flexarray *flex,
                                           int16_t bin, int16_t index)
{
    (void)flex;
...
static inline int16_t flexarray_size(const struct flexarray *flex)
{
    return flex->item_count;
}

static int16_t flexarray_set(struct flexarray *flex, int16_t index, void *data)
{
    int16_t bin = flexarray_get_bin(flex, index);
    if (bin < 0)
        return -EINVAL;
    if (bin >= flex->bin_count) {
        ...
        void ***bins;
        handle h2;
        Handle h = NewHandle((flex->bin_count + 1) * sizeof(*flex->bins),
                             userID, attrLocked, NULL);
        if (toolerror()) {
            return -ENOMEM;
        }
        h2 = FindHandle((ptr) flex->bins);
        HandToHand(h2, h, GetHandleSize(h2));
        HUnlock(h2);
        DisposeHandle(h2);
        bins = (void ***)*h;
#else
        ...
        flex->bins = bins;
#ifdef __ORCAC__
        h = NewHandle(flexarray_get_bin_size(flex, flex->bin_count - 1)*sizeof(void *),
                             userID, attrLocked, NULL);
        if (toolerror()) {
            flex->bin_count--;
            return -ENOMEM;
        }
        flex->bins[flex->bin_count - 1] = (void **)*h;
        memset(flex->bins[flex->bin_count - 1], 0, 
               flexarray_get_bin_size(flex, flex->bin_count - 1) * sizeof(void *));
#else
...
static inline int16_t flexarray_append(struct flexarray *flex, void *data)
{
    return flexarray_set(flex, flexarray_size(flex), data);
}

static inline void *flexarray_get(const struct flexarray *flex, int16_t index)
{
    int16_t bin;
...
static ssize_t dstr_ensure(struct dstr *str, size_t len)
{
    ...
    if (!str->data && len <= sizeof(str->static_data))
        ...
        if (str->data) {
            ...
            str->data = new_data;
#else
            Handle h2 = NewHandle(new_len, userID, attrLocked, NULL); 
            if (toolerror()) {
                return -ENOMEM;
            }
            Handle h = FindHandle(str->data);
            HUnlock(h);
            ...
            if (!str->data)
                return -ENOMEM;
#else
            str->data = *NewHandle(new_len, userID, attrLocked, NULL); 
            if (toolerror())
                return -ENOMEM;
#endif
            if (str->used_len)
...
static int16_t dstr_printf(struct dstr *str, const char *fmt, ...)
...
static void dstr_free(struct dstr *str)
{
    if (str->data)
        free(str->data);
#ifdef __ORCAC__
    memset(str, 0, sizeof(struct dstr));
#else
    *str = INIT_DSTR;
#endif
}
...
#ifndef SKIP_ATTRIBUTE
static int pdf_set_err(struct pdf_doc *doc, int16_t errval, const char *buffer,
...
static struct pdf_object *pdf_get_object(const struct pdf_doc *pdf, int16_t index)
{
...
{
    int16_t index = flexarray_append(&pdf->objects, obj);
...
static void pdf_object_destroy(struct pdf_object *object)
{
    ...
    case OBJ_stream:
    case OBJ_image:
        dstr_free(&object->u.stream);
        break;
    case OBJ_page:
        flexarray_clear(&object->u.page.children);
        break;
    case OBJ_info:
        free(object->u.info);
        break;
    case OBJ_bookmark:
        flexarray_clear(&object->u.bookmark.children);
        break;
    }
    free(object);
}

static struct pdf_object *pdf_add_object(struct pdf_doc *pdf, int16_t type)
{
...
#ifdef __ORCAC__
    h = NewHandle(sizeof(*obj), userID, attrLocked, NULL);
    if (toolerror()) {
...
static void pdf_del_object(struct pdf_doc *pdf, struct pdf_object *obj)
{
    ...
    if (pdf->last_objects[type] == obj) {
        ...
        for (int16_t i = 0; i < flexarray_size(&pdf->objects); i++) {
                           ...
                           const struct pdf_info *info)
{
...
#ifdef __ORCAC__
    TimeRec curTime;
    Handle h = NewHandle(sizeof(struct pdf_doc), userID, attrLocked, NULL);
    pdf = (struct pdf_doc *) *h;
    if (toolerror()) {
        return NULL;
    }
    memset(pdf, 0, sizeof(struct pdf_doc));
#else
...
#ifdef __ORCAC__
    h = NewHandle(sizeof(struct pdf_info), userID, attrLocked, NULL);
    obj->u.info = (struct pdf_info *) *h;
    if (toolerror()) {
        pdf_destroy(pdf);
        return NULL;
    }
    memset(obj->u.info, 0, sizeof(struct pdf_info));
#else
    obj->u.info = (struct pdf_info *)calloc(sizeof(*obj->u.info), 1);
    if (!obj->u.info) {
        pdf_destroy(pdf);
        return NULL;
    }
    ...
    if (info) {
        *obj->u.info = *info;
        obj->u.info->creator[sizeof(obj->u.info->creator) - 1] = '\0';
        obj->u.info->producer[sizeof(obj->u.info->producer) - 1] = '\0';
        obj->u.info->title[sizeof(obj->u.info->title) - 1] = '\0';
        obj->u.info->author[sizeof(obj->u.info->author) - 1] = '\0';
        obj->u.info->subject[sizeof(obj->u.info->subject) - 1] = '\0';
        obj->u.info->date[sizeof(obj->u.info->date) - 1] = '\0';
    }
    ...
    if (!obj->u.info->date[0]) {
        ...
        localtime_r(&now, &tm);
#endif
        strftime(obj->u.info->date, sizeof(obj->u.info->date), "%Y%m%d%H%M%SZ",
                 &tm);
#else
        curTime = ReadTimeHex();
        snprintf(obj->u.info->date, sizeof(obj->u.info->date), "20%02d%02d%02d%02d%02d%02d",
...
int pdf_set_font(struct pdf_doc *pdf, const char *font)
{
    ...
    for (obj = pdf_find_first_object(pdf, OBJ_font); obj; obj = obj->next) {
        if (strcmp(obj->u.font.name, font) == 0)
            break;
        last_index = obj->u.font.index;
    }
    ...
    if (!obj) {
        ...
        if (!obj)
            return pdf->errval;
        strncpy(obj->u.font.name, font, sizeof(obj->u.font.name));
        obj->u.font.name[sizeof(obj->u.font.name) - 1] = '\0';
        obj->u.font.index = last_index + 1;
    }
...
struct pdf_object *pdf_append_page(struct pdf_doc *pdf)
{
    ...
    page->u.page.width = pdf->width;
    page->u.page.height = pdf->height;
                      ...
                      float width, float height)
{
    ...
    page->u.page.width = width;
    page->u.page.height = height;
    return 0;
}
...
static int pdf_get_bookmark_count(const struct pdf_object *obj)
{
    ...
    if (obj->type == OBJ_bookmark) {
        int16_t nchildren = flexarray_size(&obj->u.bookmark.children);
        count += nchildren;
        for (int16_t i = 0; i < nchildren; i++) {
            count += pdf_get_bookmark_count(
                (const struct pdf_object *)flexarray_get(
                    &obj->u.bookmark.children, i));
        }
    }
    return count;
}

static int pdf_save_object(struct pdf_doc *pdf, FILE *fp, int16_t index)
{
    ...
    case OBJ_stream:
    case OBJ_image: {
        fwrite(dstr_data(&object->u.stream), dstr_len(&object->u.stream), 1, fp);
        break;
    }
    case OBJ_info: {
        struct pdf_info *info = object->u.info;
    ...
    case OBJ_page: {
        ...
        fprintf(fp, ">>\r\n");
        fprintf(fp, "/Contents [\r\n");
        for (int16_t i = 0; i < flexarray_size(&object->u.page.children); i++) {
            struct pdf_object *child =
                (struct pdf_object *)flexarray_get(&object->u.page.children, i);
            fprintf(fp, "%d 0 R\r\n", child->index);
        }
    ...
    case OBJ_bookmark: {
        ...
        parent = object->u.bookmark.parent;
        if (!parent)
            parent = pdf_find_first_object(pdf, OBJ_outline);
        if (!object->u.bookmark.page)
            break;
        fprintf(fp,
                "<<\r\n"
                ...
                "/Parent %d 0 R\r\n"
                "/Title (%s)\r\n",
                object->u.bookmark.page->index, pdf->height, parent->index,
                object->u.bookmark.name);
        int16_t nchildren = flexarray_size(&object->u.bookmark.children);
        if (nchildren > 0) {
            struct pdf_object *f, *l;
            f = (struct pdf_object *)flexarray_get(&object->u.bookmark.children,
                                                   0);
            l = (struct pdf_object *)flexarray_get(&object->u.bookmark.children,
                                                   nchildren - 1);
        ...
        for (other = object->prev;
             other && other->u.bookmark.parent != object->u.bookmark.parent;
             other = other->prev)
            ;
        if (other)
...
static uint64_t hash(uint64_t hashVal, const void *data, size_t len)
{
    const uint8_t *d8 = (const uint8_t *)data;
    for (; len; len--) {
        hashVal = (((hashVal & 0x07ffffffffffffff) << 5) + hashVal) + *d8++;
    }
    return hashVal;
}
...
int pdf_save_file(struct pdf_doc *pdf, FILE *fp)
{
    ...
    id1 = hash(5381, obj->u.info, sizeof(struct pdf_info));
                          ...
                          const char *buffer)
{
    ...
    dstr_printf(&obj->u.stream, "<< /Length %zu >>stream\r\n", len);
    dstr_append_data(&obj->u.stream, buffer, len);
    dstr_append(&obj->u.stream, "\r\nendstream\r\n");

    return flexarray_append(&page->u.page.children, obj);
}

#ifndef __ORCAC__
int pdf_add_bookmark(struct pdf_doc *pdf, struct pdf_object *page, int16_t parent,
                     const char *name)
{
    ...
    strncpy(obj->u.bookmark.name, name, sizeof(obj->u.bookmark.name));
    obj->u.bookmark.name[sizeof(obj->u.bookmark.name) - 1] = '\0';
    obj->u.bookmark.page = page;
    if (parent >= 0) {
        struct pdf_object *parent_obj = pdf_get_object(pdf, parent);
        if (!parent_obj)
            return pdf_set_err(pdf, -EINVAL, "Invalid parent ID %d supplied",
                               parent);
        obj->u.bookmark.parent = parent_obj;
        flexarray_append(&parent_obj->u.bookmark.children, obj);
    }
                                ...
                                float yoff, uint32_t colour, float spacing)
{
    ...
    dstr_append(&str, "BT ");
    dstr_printf(&str, "/GS%ld gs ", alpha);
    dstr_printf(&str, "%f %f TD ", xoff, yoff);
    dstr_printf(&str, "/F%d %f Tf ", pdf->current_font->u.font.index, size);
...
static 
#endif
int pdf_text_point_width(struct pdf_doc *pdf, const char *text,
                                ...
                                const uint16_t *widths, float *point_width)
{
    ...
    *point_width = 0.0f;
    ...
    for (int i = 0; i < (int)text_len;) {
        ...
        int16_t code_len;
#ifdef __ORCAC__
        /* handle Webbers use of 0xCA for spaces using SIS fonts */
        if (text[i] == 0xCA) {
            char txt = 0x20;
            code_len = utf8_to_utf32(&txt, text_len - i, &code);
        } else
#endif
...
static 
#endif
const uint16_t *find_font_widths(const char *font_name)
{
...
int pdf_add_text_wrap(struct pdf_doc *pdf, struct pdf_object *page,
                      const char *text, float size, float xoff, float yoff,
                      uint32_t colour, float wrap_width, int16_t align,
                      float *height)
{
    ...
    widths = find_font_widths(pdf->current_font->u.font.name);
    if (!widths)
        return pdf_set_err(pdf, -EINVAL,
                           "Unable to determine width for font '%s'",
                           pdf->current_font->u.font.name);
...
int pdf_add_custom_path(struct pdf_doc *pdf, struct pdf_object *page,
                        ...
                        int16_t operation_count, float stroke_width,
                        uint32_t stroke_colour, uint32_t fill_colour)
{
    ...
    for (int16_t i = 0; i < operation_count; i++) {
...
int pdf_add_polygon(struct pdf_doc *pdf, struct pdf_object *page, float x[],
                    float y[], int16_t count, float border_width, uint32_t colour)
{
    int16_t ret;
    struct dstr str = INIT_DSTR;
...
static int16_t find_128_encoding(char ch)
{
    for (int16_t i = 0; i < (int)ARRAY_SIZE(code_128a_encoding); i++) {
...
static float pdf_barcode_128a_ch(struct pdf_doc *pdf, struct pdf_object *page,
                                 float x, float y, float width, float height,
                                 uint32_t colour, int16_t index, int16_t code_len)
{
                                ...
                                const char *string, uint32_t colour)
{
    ...
    for (i = 1, s = string; *s; s++, i++) {
        int16_t index = find_128_encoding(*s);
...
static int16_t find_39_encoding(char ch)
{
    for (int16_t i = 0; i < (int)ARRAY_SIZE(code_39_encoding); i++) {
                              ...
                              const char *string, uint32_t colour)
{
    ...
    e = pdf_barcode_39_ch(pdf, page, newx, y, char_width, height, colour, '*', &newx);
    if (e < 0)
        return e;

    while (string && *string) {
        e = pdf_barcode_39_ch(pdf, page, newx, y, char_width, height, colour,
                              *string, &newx);
        if (e < 0)
            return e;
        string++;
...
int pdf_add_barcode(struct pdf_doc *pdf, struct pdf_object *page, int16_t code,
                    float x, float y, float width, float height,
                    const char *string, uint32_t colour)
{
    if (!string || !*string)
...
static int16_t jpeg_details(const unsigned char *data, size_t data_size,
                        int16_t *width, int16_t *height, int16_t *ncolours)
{
    ...
    for (size_t i = 0; i < data_size - 3; i++) {
        ...
        if (data[i] == 0xff && (data[i + 1] & 0xf0) == 0xc0) {
            int16_t len = data[i + 2] * 256 + data[i + 3];
                         ...
                         size_t *length)
{
    ...
    uint32_t len;
#ifdef __ORCAC__
    OpenRecGS openRec = { 12, 0, NULL, readEnable, dataForkNum };
    IORecGS readRec = { 4 };
    RefNumRecGS closeRec = { 1 };
    GSString255 pathname;
    handle h;

    strcpy(pathname.text, file_name);
    pathname.length = strlen(file_name);

    openRec.optionList = NULL;
    OpenGS(&pathname);
    if (toolerror()) {
        pdf_set_err(pdf, toolerror(), "Unable to open %s: %s", file_name,
                    "");
        return NULL;
    }
    len = openRec.eof;

    h = NewHandle(len, userID, attrLocked, NULL);
    closeRec.refNum = openRec.refNum;
    readRec.refNum = openRec.refNum;
    readRec.dataBuffer = *h;
    readRec.requestCount = len;
    ReadGS(&readRec);

    CloseGS(&closeRec);

    file_data = readRec.dataBuffer;
                                         ...
                                         size_t len)
{
    ...
    dstr_printf(&obj->u.stream,
                ...
                (int)len);
    dstr_append_data(&obj->u.stream, jpeg_data, len);
                            ...
                            size_t len)
{
...
#else
    final_data = (uint8_t *) *NewHandle(info.length, userID, attrLocked, NULL); 
    if (toolerror()) {
        pdf_set_err(pdf, -ENOMEM, "Unable to allocate PNG data %ld",
                    info.length + 1024);
        goto info_free;
    }
#endif
    ...
    dstr_append_data(&obj->u.stream, final_data, written);
...
#else 
        bmp_data = (uint8_t *) *NewHandle(data_len, userID, attrLocked, NULL); 
        if (toolerror()) {
            return pdf_set_err(pdf, -ENOMEM,
                               "Insufficient memory for bitmap");
        }
...
#else
        uint8_t *line = (uint8_t *) *NewHandle(width * 3, userID, attrLocked, NULL); 
        if (toolerror()) {
            free(bmp_data);
            return pdf_set_err(pdf, -ENOMEM,
                               "Unable to allocate memory for bitmap mirror");
...
static int16_t header_to_image(const uint8_t *data, size_t length)
{
